...
MODEL_METRICS = {
    "LightGBM":            {"auc": "0.7685", "recall": "0.6862", "f1": "0.2759"},
    "XGBoost":             {"auc": "0.7634", "recall": "0.6812", "f1": "0.2712"},
    "Random Forest":       {"auc": "0.7441", "recall": "0.6518", "f1": "0.2624"},
    "Logistic Regression": {"auc": "0.7437", "recall": "0.6485", "f1": "0.2619"},
}
...
with st.sidebar:
    ...
    selected_model = st.selectbox(
        ...
        help="All 4 models trained on same data. LightGBM is best performer."
    )
    model, imputer_medians = load_artifacts(selected_model)
    m = MODEL_METRICS[selected_model]
    st.caption(f"AUC: **{m['auc']}** · Recall: **{m['recall']}** · F1: **{m['f1']}**")
    st.markdown("---")
...
st.markdown(f"Powered by **{selected_model}** · 237 features · SHAP explainability")
st.markdown("---")
...
with tab3:
    ...
    st.markdown("#### 📊 All Models — Test Set Performance")
    compare_df = pd.DataFrame(MODEL_METRICS).T.reset_index()
    compare_df.columns = ["Model", "ROC-AUC", "Recall (Default)", "F1 (Default)"]
    compare_df["Selected"] = compare_df["Model"].apply(lambda x: "✅" if x == selected_model else "")
    ...
    with col2:
        ...
        **Key Insight:**
        - Recall on defaults is the critical metric
        - LightGBM & XGBoost outperform tree baselines
        - EXT_SOURCE scores are top predictors across all models
        """)
    st.caption("Built with Streamlit · Scikit-learn · XGBoost · LightGBM · SHAP  |  Portfolio Project by Naga")